...
describe('modeToNum', () => {
  test('3days', () => {
    const mode = 'timeThreeDays';
    const num = utils.modeToNum(mode);
    expect(num).toEqual(3);
  });

  test('week', () => {
    const mode = 'timeGridWeek';
    const num = utils.modeToNum(mode);
...
describe('spanning events', () => {
  test('first day', () => {
    ...
    const { isMultipleDays, isMultipleDaysStart, eventWeekDuration } =
      utils.getEventSpanningInfo(events[7], date, date.day(), 0);
